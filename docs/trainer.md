...
def get_batch(split):
    ...
    ix = torch.randint(len(ds) - block_size, (batch\_size,))  # 64 случайных позиции
...
| :--- | :---: | :--- |
| `batch\_size` | 64 | Количество примеров в одном батче |
...
\text{lr}_{max} \times \frac{t}{T_{warmup}} & \text{если } t < T_{warmup} \\
...
graph TD
    ...
    GA --> CLIP["Gradient Clipping<br/>max\_norm = 1.0"]
...
где:
- $N$ — общее количество предсказаний (batch\_size × block_size)
...
```python
gradient_accumulation\_steps = 2  # Эффективный батч = 64 × 2 = 128
```

На каждой итерации:
1. Выполняются **2 прямых прохода** с разными мини-батчами.
2. Потеря каждого прохода **делится на 2**: `loss = loss / gradient_accumulation\_steps`.
...
```python
torch.nn.utils.clip_grad_norm_(model.parameters(), max\_norm=1.0)
```
...
flowchart TD
    ...
    ACCUM_START -->|Нет| CLIP["✂️ Clip gradients<br/>max\_norm = 1.0"]
    CLIP --> STEP["optimizer.step()"]
    STEP --> LOOP_START
...
| `batch\_size` | 64 | Примеров в мини-батче |
