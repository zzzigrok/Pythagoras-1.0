...
def print_banner():
    banner = r"""
 [bold cyan]
  _____ __     __ _______ _    _          _____  ____  _____         _____ 
 |  __ \\ \   / /|__   __| |  | |   /\   / ____|/ __ \|  __ \   /\   / ____|
 | |__) |\ \_/ /    | |  | |__| |  /  \ | |  __| |  | | |__) | /  \ | (___  
 |  ___/  \   /     | |  |  __  | / /\ \| | |_ | |  | |  _  / / /\ \ \___ \ 
 | |       | |      | |  | |  | |/ ____ \ |__| | |__| | | \ \/ ____ \____) |
 |_|       |_|      |_|  |_/_/    \_\_____|\____/|_|  \_\/    \_\_____/
 [/]
 [bold blue]
 ──────────────────── [v2.2 QUANTUM NEURAL ENGINE] ────────────────────
 [/]
    """
...
def mode_chat():
    ...
    if not os.path.exists('math_model_weights.pth'):
...
def mode_history():
    ...
    if not os.path.exists('math_chat_history.txt'):
        ...
        with open('math_chat_history.txt', 'r', encoding='utf-8') as f:
...
def mode_debug():
    ...
    while True:
        ...
        if choice == "1":
            ...
            with open('math_vocab.pkl', 'rb') as f: v_data = pickle.load(f)
            m = SimpleLLM(v_data['vocab_size']).to(device)
            m.load_state_dict(torch.load('math_model_weights.pth', map_location=device, weights_only=True))
